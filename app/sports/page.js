...
export default function Page() {
  ...
  const fetchRecentBets = async () => {
    try {
      const res = await fetch("/api/getBets?username=lokesh");
      const data = await res.json();
      if (data.success) {
          ...
          <div className="max-w-[1200px] mx-auto px-4 py-6">
            ...
            <div className="flex gap-3">
              <div className="flex items-center gap-1.5 bg-white/15 px-3 py-1.5 rounded-full text-sm">
                ...
                {matchdata.filter((m) => m.iplay).length} Live
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:w-[320px] w-full shrink-0">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sticky top-6">
              ...
              <div className="bg-gray-50 px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 className="font-bold text-[#0B3B8A] flex items-center gap-2">
              ...
              <div className="p-4 space-y-3 max-h-[500px] overflow-y-auto">
                {recentBets.length > 0 ? (
                  recentBets.map((bet) => (
                    <div key={bet.id} className="bg-gray-50 rounded-xl p-3 border border-gray-100 hover:border-blue-200 hover:shadow-sm transition-all">
                      ...
                      <div className="mt-2 flex justify-between items-center pt-2 border-t border-gray-200/60">
                        <div className="flex gap-4">
                          <div className="flex flex-col">
                            <span className="text-[10px] uppercase text-gray-400 font-bold">Odds</span>
                            <span className="font-bold text-gray-900">{bet.odds}</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[10px] uppercase text-gray-400 font-bold">Stake</span>
                            <span className="font-bold text-gray-900">₹{bet.stake}</span>
                          </div>
                        </div>
