...
function BetSlip({ selection, onPlace, onClose }) {
  const [stake, setStake] = useState("");
  const [placing, setPlacing] = useState(false);
  const isBlue = selection.betType === "back";
  const profit =
    stake && selection.odds
      ? (parseFloat(stake) * (selection.odds - 1)).toFixed(2)
      : "0.00";
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="px-3 py-1.5 rounded-lg border border-gray-300 w-24 font-bold text-center text-sm"
            autoFocus
          />
          <button
            onClick={onClose}
...
function OddsBtn({ value, size, isBack, onClick, isSelected }) {
  const base = isBack
  ...
  return (
    <button
      ...
      className={`min-w-[54px] rounded-md px-2.5 py-1.5 text-center transition cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${base} ${ring}`}
    >
          ...
          <div className="flex flex-col gap-3">
            {bets.map((bet, i) => {
              ...
              return (
                <div
                  key={i}
                  className="bg-gray-50 rounded-xl p-3 border border-gray-100 hover:border-blue-200 transition-all"
                >
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span
                      className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${statusColor}`}
                    >
                      {bet.status}
                    </span>
                  </div>
                  ...
                  <div className="mt-2 flex justify-between items-center pt-2 border-t border-gray-200/60">
...
export default function CricketBettingPage({ params }) {
  ...
  const fetchMatchDetails = useCallback(async () => {
    try {
      const res = await fetch(
        `http://130.250.191.212:3009/getPriveteData?gmid=${id}&sid=4&key=${process.env.NEXT_PUBLIC_SPORTS_API_KEY}`
      );
  ...
  useEffect(() => {
    ...
    //   try {
    //     const res = await fetch(`http://130.250.191.212:3009/getPriveteData?gmid=${id}&sid=4&key=${process.env.NEXT_PUBLIC_SPORTS_API_KEY}`);
    //     const data = await res.json();
    //     const all = data.data || [];
    //     setMatchdata(all);
    //     setMatchodds(all.find((m) => m.mname === "MATCH_ODDS") || null);
  ...
  const handlePlaceBet = async (betData) => {
    ...
    try {
      const externalRes = await fetch(
        `http://130.250.191.212:3009/placed_bets?key=${process.env.NEXT_PUBLIC_SPORTS_API_KEY}`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            event_id: Number(body.event_id) || body.event_id,
            event_name: body.event_name,
            market_id: Number(body.market_id) || body.market_id,
      ...
      console.error("External bet placement error", e);
    }
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-blue-200 text-sm">
                Live matches &amp; upcoming fixtures
              </p>
            </div>
          </div>
          {!loading && matchodds && (
